                    ...
                    Route53ConnectorOp::CreateResourceRecordSet(record_set) => {
                        ...
                        if let Some(resource_records) = record_set.resource_records {
                            for rec in resource_records {
                                let resource_record_builder =
                                    aws_sdk_route53::types::ResourceRecord::builder().value(rec);
                                record_set_builder = record_set_builder
                                    .resource_records(resource_record_builder.build()?);
                            }
                        }

                        if let Some(alias_target) = record_set.alias_target {
                            let alias_target_builder =
                                AliasTarget::builder().dns_name(alias_target);
                            record_set_builder =
                                record_set_builder.alias_target(alias_target_builder.build()?);
                        }
