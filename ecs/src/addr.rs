    ...
    fn to_path_buf(&self) -> PathBuf {
        match &self {
            ...
            EcsResourceAddress::TaskDefinition(region, task_def_id) => {
                PathBuf::from(format!(
                    "aws/ecs/{}/task_definitions/{}.ron", 
                    region, task_def_id
                ))
            }
            EcsResourceAddress::Task(region, cluster_name, task_id) => {
                PathBuf::from(format!(
                    "aws/ecs/{}/clusters/{}/tasks/{}.ron",
                    region, cluster_name, task_id
                ))
            }
    ...
    fn from_path(path: &Path) -> Result<Self, anyhow::Error> {
        ...
        match &path_components[..] {
            ["aws", "ecs", region, "clusters", cluster_name] if cluster_name.ends_with(".ron") => {
                ...
                Ok(EcsResourceAddress::Cluster(
                    region.to_string(),
                    cluster_name,
                ))
            }
            ["aws", "ecs", region, "clusters", cluster_name, "services", service_name]
                if service_name.ends_with(".ron") =>
            {
            ...
            ["aws", "ecs", region, "task_definitions", task_def_id] if task_def_id.ends_with(".ron") => {
                ...
                Ok(EcsResourceAddress::TaskDefinition(
                    region.to_string(),
                    task_def_id,
                ))
            }
            ["aws", "ecs", region, "clusters", cluster_name, "tasks", task_id]
                if task_id.ends_with(".ron") =>
            {
            ...
            ["aws", "ecs", region, "clusters", cluster_name, "container_instances", container_instance_id]
