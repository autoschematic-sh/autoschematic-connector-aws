...
pub struct EcsConnectorConfig {
    pub enabled_regions: Vec<String>,
}

impl Default for EcsConnectorConfig {
    fn default() -> Self {
        Self {
            enabled_regions: vec![
                // "af-south-1",
...
impl EcsConnectorConfig {
    pub fn try_load(prefix: &Path) -> anyhow::Result<Option<EcsConnectorConfig>> {
        let config_path = prefix.join("aws/ecs/config.ron");
